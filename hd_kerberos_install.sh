...
for client_name in ${client_list}; do
	...
	echo "==========================================="
	ssh ${client_user}@${client_name} -t "sudo scp -r ${master_user}@${master_name}:/opt/hadoop/ /opt/ > /dev/null && sudo mkdir -p /opt/key/ &&sudo scp -r ${master_user}@${master_name}:/opt/key/ /opt/key/ > /dev/null && sudo chown -Rf ${client_user}.${client_user} /opt/key"
	echo "==========================================="
