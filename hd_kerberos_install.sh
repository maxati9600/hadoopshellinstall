...
for client_name in ${client_list}; do
	...
	echo "==========================================="
	ssh ${client_user}@${client_name} -t "sudo scp -r ${master_user}@${master_name}:/opt/hadoop/ /opt/ > /dev/null && sudo mkdir -p /opt/key/ca &&sudo scp -r ${master_user}@${master_name}:/opt/key/ca/ /opt/key/ca/ > /dev/null && sudo chown -Rf ${client_user}.${client_user} /opt/key"
	echo "==========================================="
